...
正面に並んだ窓を持つ 9×7 のコテージ — コンパイラがブロックを組み立てます。階段の向き、ドアの
蝶番、板ガラスの接続、座標計算、そしてエディションとバージョンごとに異なるブロック ID まで。
...
- tree-sitter 文法と WebAssembly バインディング (後者は
  [プレイグラウンド](https://cairn.kage1020.com/ja/playground/)を動かしています)
...
成果物へのレッドストーン出力、およびそれを検証するティックシミュレータ。

## 核となる考え方

- **ブロックステートではなく意図を書く。** 階段の向き、ドアの向き、板ガラスの接続、ベッドの頭と
  足は導出されます。上書きするのは、その値自体が意図であるときだけです。
...
リリースは日付ベースのバージョニング `YYYY.M[.PATCH]` を使い、言語・コンパイラ・標準ライブラリ・
レジストリパックをひとつの束として扱います。これは `--target` に渡す Minecraft のバージョンとは
