...
- *(cli)* `cairn check` が `--target <version>` を受け取り、`E_UNKNOWN_ID` を報告するようになりま
  ...
  そのため `cairn check` で門番をする CI ジョブは、`cairn compile` が拒否するソースに対して緑になっ
  ていました。これは `E_PARTIAL_BUILD` と `compile` 内での `check` 呼び出しが塞ごうとしたのと同じ形の
  穴で、ここではそれが本質的でした。検査に必要な情報がコマンドラインに無かったからです。このフラグが
  それを置きます。仕様 §4.2 が `--target` 単独を禁じるのと同じ理由で `--edition` を必須とし
  (「1.21」は Java と Bedrock で別のリリースを指します)、`latest` エイリアスを含めて `compile` と同じ
  データテーブルに対して解決します。エディションが出荷していないバージョンは `compile` と同じ拒否に
  なり、コマンドラインのタイポが構文エラーを埋もれさせないよう、ファイル自身の指摘の後に印字されます。
  ...
  だからです。付いてこないのは `compile` が書くものです。成果物もロックファイルも作らず、`@requires`
  のフロア強制も行いません。ロックファイルはビルドを証明するものであり、`--target` をフロアに突き合わ
  せるのはそれを生成するコマンドの仕事です。
