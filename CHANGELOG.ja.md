...
- *(redstone)* `W_WIRE_CROSSING` は 1 つのダスト座標を共有する 2 つの信号を名指しします。交差の
  ...
  は 1 件です。redstone の例は 2 つとも 1 件ずつ該当します — `redstone-door.crn` と
  `crossbar.crn` の `--stage crossing` は信号が混ざる場所を表示するようになり、終了コードは従来
  どおり 0 です。報告であって修復ではないため、座標は 1 つも動きません。
