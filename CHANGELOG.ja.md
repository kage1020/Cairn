...
- *(cli)* `cairn info` が、どのサポート対象ターゲットならビルドできるかを報告するようになりました。
  ...
  これはポータビリティのカウンタが既に従っている規則と同じです。拒否し、どのブロックが無いのかを
  言うのはビルドのほうです。**破壊的**: `compute_axes` の引数が 5 つになり、`VersionAxes` に 5 つ目の
  フィールドが増えます。`--format json` には `buildable_targets` が加わりますが、未知のキーを無視する
  消費者にとっては additive です。
