...
- *(cli)* `cairn check` が `--target <version>` を受け取り、`E_UNKNOWN_ID` を報告するようになりま
  した。これまでこのコマンドは `--edition` しか取らず、バージョンを固定していませんでした。ブロック
  ID はあるバージョンに存在するかしないかのどちらかなので、何も固定されていない状態では、誰も選んで
  いないバージョンに対して照合する代わりに検査自体が飛ばされていました。
  ...
  そのため `cairn check` をゲートにする CI ジョブは、`cairn compile` が拒否するソースに対して緑に
  なっていました。これは `E_PARTIAL_BUILD` と `compile` 内での `check` 呼び出しが塞ぐために追加され
  たのと同じ形の穴で、ここではそれが本質的でした。検査に必要な情報がコマンドラインに無かったからです。
  このフラグがそれを置きます。仕様 §4.2 が `--target` 単独を禁じるのと同じ理由で `--edition` を必須と
  し (「1.21」は Java と Bedrock で別のリリースを指します)、`latest` エイリアスを含めて `compile` と
  同じデータテーブルに対して解決します。エディションが出荷していないバージョンは `compile` と同じ拒否
  になり、コマンドラインのタイポが構文エラーを埋もれさせないよう、ファイル自身の指摘の後に印字されま
  す。その際もバージョン未固定のビューで lowering は走るので、ID テーブルを必要としない指摘は 2 回目
  の実行を待たずにその場で報告されます。

  バージョンの固定が block-array lowering を有効にするので、その段の他の指摘 —
  `E_UNKNOWN_ABSTRACT_TOKEN`、`E_INCOMPATIBLE_MATERIAL` — も一緒に付いてきます。フィルタで落とすこと
  はしません。パスを走らせて見つけておきながら黙るのは、このフラグが終わらせようとしている沈黙そのもの
  だからです。lowering がスコープを落とせば `compile` と同じ `E_PARTIAL_BUILD` で拒否します。理由は
  同じで言葉だけが違います。compile は部分ビルドを証明してはならず、check は何も証明しない代わりに、
  その固定で compile が拒否するソースを通してはならないからです。付いてこないのは `compile` が書くもの
  です。成果物もロックファイルも作らず、`@requires` の下限の強制 (`E_VERSION_CAP`) も行いません。
  ロックファイルはビルドを保証する記録であり、`--target` を下限に突き合わせるのはそれを生成するコマン
  ドの仕事です。

  この 2 つの実行レベルの拒否 — 出荷されていないターゲットと、失われたスコープ — はどちらも
  `--format json` の配列の要素にはなりません。どちらもファイル中のスパンに紐づく指摘ではなく、行番号を
  与えれば持っていない位置を騙ることになるからです。両方とも stderr と終了コードで報告します。
  `compile` がそれらに与えている形と同じです。

  エディションが出荷する全バージョンに照合する方式ならフラグは不要でしたが、答えるのは別の問いです。
  `stone_bricks` は Bedrock 1.21.40 のブロックで 1.21.0 のブロックではないので、エディション単位の
  パスはどこでもそれを受け入れ、1.21.0 のビルドには何も伝えません。そここそが面白い方の半分であり、
  固定はそれを正しく扱います。拒否はパックのエイリアステーブルから `minecraft:stonebrick` を名指し
  します。

  フラグはオプトインなので、ターゲット無しの挙動は変わらず、今日通っているソースが落ち始めることは
  ありません。また `DiagnosticData::UnknownId` (`spec/lint.md` §11.2) が CLI から初めて観測可能になり
  ます。このペイロードは文書化されていながらライブラリ利用者にしか届いていませんでした。`compile` は
  テキストを印字し、`check --format json` はこのコードを一度も出さなかったからです。

- *(core)* `@cairn` ヘッダの値が言語バージョンとして読まれるようになりました。コードは 2 つで、
  値が `YYYY.M[.PATCH]` になっていないときの `W_INVALID_CAIRN_VERSION` と、読んでいるコンパイラより
  新しいバージョンを指しているときの `W_FUTURE_CAIRN_VERSION` です。`@cairn banana` も
  `@cairn 2026.06.1.2` も `@cairn 2026.13` も `@cairn 9999.1` も、これまで無言で受け付けられて
  いました。パイプラインの誰もその文字列を見ていなかったからです。パース時にそのまま包まれ、その後
  読むのは重複ヘッダのパスが名前を欲しがるときの 1 回だけでした。

  このヘッダは、後のコンパイラが正しくパース/警告できるようにするために存在します。どのコンパイラにも
  読めない provenance は、その仕事を果たせません。future 側のコードはその裏返しで、自分より新しい言語
  で書かれたファイルについて古いビルドが言える唯一の有用なことです。このビルドより後に追加された
  キーワードや引数は `E_UNKNOWN_KEYWORD` / `E_UNKNOWN_ARGUMENT` として報告されるので、それらがその行
  についてではなくバージョン差についての指摘かもしれない、と知っているのはヘッダだけです。まったく
  新しい構文形式は `E_PARSE` になり、そのとき check パスは 1 つも走らないので、バージョン差が最も
  説明になる場面ではこの指摘は出ません。

  どちらもエラーではなく **警告** で、これは `@requires` とは逆の判断です。`spec/lint` §11.3 は
  「放置するとソースが求めたものと違うものが出る」ときにエラーとします。`@cairn banana` は
  `@cairn 2026.06` とバイト単位で同じものを建てます。`@requires` が同じ判定でエラーなのは、その下限が
  入力そのものだからです。`cairn info` の互換範囲を決め、`cairn compile --target` がそれに縛られるので、
  蒸発した下限は通してはいけないターゲットを通します。今日コンパイルできるソースがコンパイルできなく
  なることはありません。

  受け付ける形は 4 桁の年、`1 … 12` の月、省略可能なパッチ、そしてその後には何も無いことです。ヘッダ
  の値は行まるごとなので、`@cairn 2026.6 draft` はバージョンとそれ以外の何かを書いていることに
  なります。月の先頭のゼロは受け付けられ、値を変えないので、`2026.06` (同梱の `.crn` も仕様自身も
  こう書きます。calver.org の `YYYY.0M` です) と `2026.6` は同じバージョンです。`@requires` が読む
  ラベルより意図的に厳しくしてあります。あちらは Mojang の名前空間のラベルに順序を付けるだけで、
  構成要素は数字で始まればよくプレリリースタグもラベルですが、こちらは Cairn 自身のバージョンを指し、
  構成要素はすべて数字、`2026.13` は存在しない月、`1.2` は年ではなく semver だからです。

### 修正

- *(docs)* `spec/versioning-editions.md` §10.7 は `@edition` エスケープハッチの例に
  `minecraft:light_block["block_light_level"=15]` を使っていました。これは Bedrock 1.21.0 にはあり、
  レジストリパックが持つ残り 2 つのターゲットには無い ID です。1.21.40 が明るさを ID の中に繰り上げた
  ので、1.21.40 と 1.21.60 はこのブロックを `light_block_0` … `light_block_15` と綴り、`light_block`
  は持ちません。この節は「Bedrock は綴りが違う」ことを示すために spec が ID を名指す唯一の場所なので、
  そこの誤った綴りが読者の持ち帰る綴りになります。そして、そこから写したソースは 3 つのうち 2 つの
  ターゲットで `E_UNKNOWN_ID` に落ちます。

  分岐は平坦化後の ID で書き直し、2 つの綴りのどちらに向けたものかを述べるエディション付き下限を隣に
...
- *(core)* パレットを、intern の履歴ではなく **正準な並べ方を持つ集合** にしました。スロット `0` は
  従来どおり空気で、それ以外は `(id, properties)` — id、次に状態プロパティを名前順で比較 — によって
  配置され、ボクセルのグリッドはその結果に番号を振り直されます。

  `spec/compilation.md` §4.1 は冒頭で、ソースを「フラットに、順不同で」書けると約束しています。
  フェーズのバケットはフェーズ間についてそれを果たし、パレットの刈り込みは最後のセルを他のメンバに
  奪われた材質について果たしますが、どちらも「同一フェーズにいて 1 つもボクセルを共有しない 2 つの
  メンバ」には届きませんでした。`Palette::intern` は初回使用時に追記し、初回使用とは塗りであり、
  フェーズ内の塗りは書かれた行の順に走るからです。結果として、向かい合う壁の 2 つの窓は行順で
  intern されていました。
  ...
  この 2 行を入れ替えると、`.nbt` がそのまま書き出すパレットの中で、`cairn info` がエントリごとに
  1 行印字する可搬性の行の中で、そして配列全体を覆う `resolved_ir_hash` の中で、`oak_planks` が
  `glass_pane` を追い越しました。ボクセルは 1 つ残らず同一なのに成果物が違う。そのハッシュを鍵に
  したビルドキャッシュは、何も変えていない編集で外れていました。

  §4.1 が「同一フェーズ内のローカル上書き」に与える last-wins はこれを覆いません。あの許可は
  *どのブロックがセルを取るか* についてのものであり、この 2 つのメンバはセルを共有しないからです。
  結果は両者の順序に依存せず、依存していたのは番号付けだけでした。出口は 2 つ — ソートするか、
  パレットの順序もソースが記述するものの一部だと言い、「順不同」はバイト列ではなくボクセルについての
  主張だと認めるか。§4.8 は前者を採ります。パレットは完成したグリッドだけから導かれるので、
  グリッドを変えないソースの並べ替えは `BlockArray` 全体を変えません。これは §4.1 が今日すでに
  読めてしまう、強い方の読み方です。

  ソートが参照するのは解決済みの状態だけで、スパン・メンバ id・フェーズ・塗った順のいずれも見ません。
  したがってこれは「2 つのターゲットのパレットが一致する」という主張ではなく、「1 つのターゲットの
  パレットがソースの並べ方に依存しない」という主張です。プロパティの袋は反復順ではなく名前順で
  比較します。`IndexMap` はマップとして比較するので、同じ組を別の順で持つ 2 つの状態はすでに等しく
  ...
  **破壊的**: Cairn が出力するすべての構造でパレットの順序が変わり、それに伴って `.nbt` のバイト列、
  ...
  (本リポジトリに例のロックファイルはコミットされていないため、ここで更新すべきものはありません)。
  ファイルに入るブロックとその位置は変わりません。動いたのは番号付けであって形状ではなく、ゲームに
  読み込んだ構造は同じ構造です。lowering パスの外で配列を組み立てる利用者のために
  `Palette::canonicalize` と `BlockArray::canonicalize_palette` を公開し、順序そのものは
  `BlockState::canonical_key` が表します。
...
- *(core,cli)* `@intended_targets` を、同じファイルが宣言するバージョン下限に照らすようになりました。
  ...
  `cairn compile --target 1.20.4` は `E_VERSION_CAP` で拒否していました。2 つの宣言のうち一方が
  ビルドを決め、もう一方は何も決めておらず、無視されていたのは指示のように読めるヘッダの方でした。

  ヘッダが名指す各バージョンは、ターゲットのエディションの `DataVersion` 表に位置づけられます。
  そのエディションのどの `--target` でもビルドできないものは `W_INTENDED_TARGET_UNSUPPORTED` です
  (パックがブロックデータを同梱していないリリースか、表が位置づけられないラベル — 後者は他方の
  エディションの採番に見えます)。これは下限には照らされません。作者がまず動くのは「そのターゲットは
  ここに存在しない」という事実だからです。残り、つまりそのエディションが *ビルドできる* バージョンは
  その中だけで数えます。そのすべてが下限を下回れば `E_INTENDED_TARGET_CAP` で、ファイルは自分が対象
  だと言っているどのバージョン向けにもビルドできません。一部なら `W_INTENDED_TARGET_CAP` です。この
  ヘッダはヒントであり (`spec/syntax.md` §5.3)、下限より上のバージョンは変わらずビルドできるからです。
  下限は合成物の畳み込みなので、ライブラリの `def` がそれを置くファイルの意図を拒否することもあり、
  所見はその構成要素を名指します。

  各コマンドは、自分が対象としているエディションの表でヘッダを照らします — `--edition` が名指す 1 つ、
  `cairn info --editions` が挙げるもの、どれも名指さない場合は両方です。1 つのエディションに絞った
  レポートが、もう一方の答えで拒否されることはありません。1 つのスパンが持つ cap 所見は 1 つで、及ぶ
  範囲について 2 つのエディションが食い違った場合はエラーを報告します。
  `W_INTENDED_TARGET_UNSUPPORTED` はスコープにエディションがちょうど 1 つある場合にだけ答えます。

  `cairn info` には `intended targets` 行が加わり、矛盾しうる相手である `buildable targets` の隣に
  並びます。従来このヘッダはレポートに一切現れず、バージョンを名指す唯一の宣言が出力に出てこない
  状態でした。

  **破壊的**: 両立しない形で 2 つのヘッダを宣言しているソースは、`cairn check` / `cairn info` /
  `cairn compile` で 0 だった終了コードが 1 になります。`VersionAxes` に `intended_targets`
...
- *(core,cli)* `def` と `theme` が独立した行として `requires version>=X` を宣言できるようになり、
  ...
  以前からそう述べていましたが、どちらの綴りもパースされず、仕様の一文はどの層にも存在しない機能を
  記述していました。`def` は `place` がインスタンス化するテンプレートなので、`def` への下限は
  それを使うすべてのサイトへの下限です。これによりテンプレートのライブラリは、利用側がその都度
  言い直さなくても自身の要求を持ち運べます。モジュールレベルの `@requires` ではこれを表現できません。
  テンプレートではなくファイル全体に適用されるからです。

  式はエディションスコープを含めて `@requires` と同じもので、同じ `parse_requirement` が読み、
  行末まで生のまま取ります。したがって `24w14a` や `1.21.4-rc1` もヘッダと同様にパースされます。
  `declared_version_floors` は合成物を歩くようになりました。モジュールのヘッダに加えて、ビルドが
  インスタンス化するすべての構成要素 — `place use=` が名指す `def` と、スコープが束縛する `theme`
  — です。`cairn info` の `registry compatibility` 行も同じ畳み込みを実行し、`E_VERSION_CAP` は
  数値だけでなく下限を課した構成要素を名指します。テンプレート内部に書かれた下限で拒否された
  ターゲットは、裸のバージョン番号だけでは対処できないからです。

  `struct` と `site` はこの行を取りません。どちらもビルドの構成要素ではなくビルドそのものなので、
  内側に書かれた下限は `@requires` がすでに述べていることになります。メンバのインデントされた子も
  同様で、そちらの修正はインデントを 1 段戻すことです。どちらの拒否も語だけを理由には発火しません。
  `requires` は何も宣言しない位置では通常のキーワードのままです。

  **破壊的**: `ast::Item` の `Def` と `Theme` の各バリアントに `requires` フィールドが、
  `resolve::VersionFloor` に `origin` フィールドが増えました。これらのバリアントの全フィールドを
  名指す外部のパターンはコンパイルできなくなります。移行は `..` の追加です。この 2 つのバリアントは
  今回から `#[non_exhaustive]` になったので、次のフィールド追加では破壊が起きません。enum 側の
  属性はバリアントの *追加* しかカバーしないからです。Rust API は Internal ティア (互換性 C.2) な
  ので猶予期間の義務はありません。ここに記載しているのは、ティアによらず C.3 が要求するためです。
  シリアライズ形は不変です。`requires` は空のときスキップされ、`VersionFloor` は `Serialize` を
  持たないので、新しい行を使わないソースについては `cairn parse` の AST JSON も `cairn info` の
  出力もバイト単位で同一です。

  以前パースできて今はできないソースが 1 つあります。`def` の本体で `requires` だけの行は引数の無い
  メンバでしたが、現在は `E_PARSE` です。下限を読む本体は、何も述べない下限の失敗を自身で扱うから
  です。いずれにせよ `cairn check` では `E_UNKNOWN_KEYWORD` でした。変わるのは `cairn parse` も
  これを拒否するようになった点です。

- *(core)* `TokenKind::Int` が保持するのは `lexeme` のみになり、`value: i64` フィールドは
  削除されました。字句解析器は整数リテラルと真理値表の行のビットパターンを区別できません。
  どちらも数字であり、どちらであるかを決めるのは周囲の文法だけだからです。そのため、どちらも
  数値へパースしなくなりました。値が必要な箇所が、その位置が実際に取る型へパースします
  (`parse_value` では `i64`、`within` の境界では `u32`)。これにより、20 入力の真理値表で行が
  `11111111111111111111` のときに拒否していた上限が外れます。整数を 1 つも含まない表に対して
  整数の範囲エラーを出す、という挙動でした。

  **破壊的**: `TokenKind` はクレートルートから再エクスポートされており、`#[non_exhaustive]` は
  variant ではなく enum に付いているため、外部の `TokenKind::Int { value, .. }` パターンは
  コンパイルできなくなります。Rust API は Internal ティア (互換性 C.2) なので非推奨期間の義務は
  ありませんが、C.3 がティアに関わらず記載を要求しているため、ここに記録します。

  値の位置にある `i64` を超える数字列は、引き続き同じ `IntContext`・lexeme・オーバーフロー種別・
  位置を持つ `E_PARSE` です。1 レイヤ後ろで上がるようになっただけです。値以外の位置では、その
  位置に応じたメッセージになります (`within` の境界なら「invalid `within` bound」、真理値表の行
  ならパターンが `0` と `1` のみを持つこと、`struct 99999999999999999999` なら識別子が必要である
  こと)。これまでは、そのすべてが整数の範囲についての 1 つのメッセージでした。

- *(core)* `connect` 行のポートは、placement が実際に塗った石積みに固定されるようになりました。
  帯とその行き先の開口を、2 つの答えではなく 1 つの答えが決めます。`walkway` はこれまで `def` の
  トップレベルの `walls` メンバとその `height=` から自前の壁カラムを組み直しており、2 つの読みは
  両方向に食い違っていました。`mat_slot=` が解決しない `walls` では開口パスが切り抜きを保留する
  一方で帯は敷かれ、切り抜かれていない窓へ、そして彫られていない戸口へ道が伸びていました
  (door の分岐はカラムを一切参照していませんでした)。逆に `level` の中で宣言された `walls` は
  塗られ、切り抜かれ、それでもアンカーとしては拒否され、既に石積みの中にある窓を動かすよう著者に
  求めていました。

  **破壊的**: 壁が何も塗らないソースは、メンバが既に得ている `W_DEFERRED_MEMBER` と並んで、
  これまで敷かれていた walkway を失います。壁が `level` の中にあるソースは、これまで拒否されて
  いた walkway を得ます。`port_world_position` も `cairn_lang_core` から公開されなくなりました。
  正しく問うには body が lowering された際のカラムが要り、それはパスの外の誰も持っていません。
  公開されていたことが、二番目の導出を招いていました。

  カラムは placement と一緒に、塗ったフェーズから読むパスへ運ばれます。二重に導出する場所が無い
  ので、規則がずれる余地もありません。level スコープの壁も解決しないマテリアルもポートの判定を
  変えますが、それは壁が変わったからであって、規則のコピーがもう 1 つ教え込まれたからではありま
  せん。行の `W_DEFERRED_MEMBER` には石積みの契約を述べる 4 つ目の note が加わります。4 つの中で
  修正箇所が別の行にある唯一の原因なので、建てられなかったメンバ自身がその行でそう述べる、と
  書いてあります。

- *(core)* `mat_slot=` を持たない `floor` / `walls` が拒否されるようになりました。新しいコード
  `E_MISSING_MATERIAL` です。どちらのロールも適用テーマのスロットマップ経由でパレットに到達し、
  既定ブロックを持たないため、スロットを書かなかったメンバは何も塗らず、しかもそれを言う場所が
  どこにもありませんでした。`cairn check` は 0 で終了し、構造は空のまま lowering されます。
  `spec/lint` §11.3 が禁じている「暗黙の drop」そのものです。

  **破壊的**: そのようなメンバを含むソースは今までコンパイルでき、今後はできません。修正は
  トークン 1 つ — 適用テーマが宣言しているスロットを名指しするだけです。同梱の example はすべて
  既にそうなっています。

  対象は「それが無いと何も置かないロール」だけです。`mat_slot=` の無い `window` や `door` は
  **開口** で、矩形が空気に彫られます。`examples/themed-tower.crn` が石壁に狭間を、樹種を選ばずに
  開けているのがこれです。`roof` / `stair` / `pressure_plate` は既定ブロックを塗ります。いずれも
  報告しません。元の報告は `window` を `floor` / `walls` と並べていましたが、そこは誤りで、
  そう言っているのは corpus です。

  実装は解決済みの束縛ではなく表層のキーを読む `check` パスです。そのおかげで、ダンプではなく
  `cairn check` の時点で出ること、テーマを 1 つも宣言しないモジュールでも「どのメンバが
  マテリアルを名指ししていないか」を伝えられること、そして `E_UNRESOLVED_SLOT` と衝突しないこと
  — あちらはキーがあって使えない場合、こちらはキーが無い場合で、両方を受け取るメンバはいません
  — が同時に成り立ちます。
...
- *(core, cli)* `@requires` の下限は、バージョンラベルをドット区切り 10 進として要素ごとに比較する
  代わりに、ターゲットのエディションのバージョン表の `DataVersion` — `spec/versioning-editions.md`
  §10.1 が正準とするキー — で順序付けられます。従来の比較は 2 つのエディションの採番を区別できません
  でした。Java は `1.20.4 / 1.21 / 1.21.4`、Bedrock は `1.21.0 / 1.21.40 / 1.21.60` と進むため、
  `@requires version>=1.21.4` は `40 > 4` で Bedrock `1.21.40` に満たされたと読まれ、ソースが排除
  しているターゲットに対して `verified: true` のロックを書いていました。下限の強制が取り除くために
  存在するのと同じ欠陥が、1 つ隣のエディションにあったわけです。

  下限は自分がどのエディションの採番で書かれているかを名乗れるようになりました
  (`@requires java version>=1.21.4`)。そのエディションのビルドを制約し、もう一方では不活性です。
  スコープ無しの下限はビルドされるエディションに対する下限で、そのエディションの表で解決されるので、
  `@requires version>=1.21` は従来どおり Java の `1.21` と Bedrock の `1.21.0` を意味します。

  表が位置づけられない下限 — 表の範囲の内側にありながらどの行でもないもの、つまり他方のエディション
  の採番で書かれた下限 — は、推測ではなく新しい `E_REQUIRES_UNORDERABLE` になります。全行より下の
  下限は全ターゲットが満たし、全行より上の下限はどのターゲットも満たしません。この 2 つは各行のキー
  が何であれ成り立つからです。

  **破壊的変更**: `resolve::declared_version_floor` は `declared_version_floors(module, edition)`
  に置き換わり、最も厳しい 1 つではなくそのエディションのビルドが従うすべての下限を返します。最も
  厳しいものを選ぶには順序が要り、その順序がエディションごとになったためです。`parse_requirement` /
  `parse_min_version` は素の `&str` ではなくエディションスコープを持つ `Requirement` を返します。
  Java の採番の下限を宣言したソースの Bedrock ビルドは、従来成功していたところで失敗します。それが
  この変更の目的です。

- *(core)* `@requires` は `spec/versioning-editions.md` §10.1 が存在すると言うバージョンラベルの形を
  すべて受け付けます。プレリリースの `1.21.4-rc1`、スナップショットの `24w14a`、そして日付ベースの
  採番が綴るものです。`-` はレキサエラーではなくトークンになったので、接尾辞がディレクティブに届くよ
  うになりました。ヘッダの値はトークン間の生のソースなので、レキサが拒否する文字はそれを受け付ける
  パスに決して届かないためです。プレリリースは、それが名指すリリースの直下に順序付けられます。ラベル
  を受け付けることは順序付けられるという主張ではありません。それはターゲットのエディションの表の答え
  で、`cairn compile --target` で問われます。`RequirementError` に `UnknownEditionScope` と
  `PreRelease` が加わり、`E_INVALID_REQUIRES` の `reason` ペイロードに `unknown_edition_scope` と
  `prerelease_not_a_tag` が加わります。

- *(formats)!* レジストリパックのバージョン表は、パックがビルドできるバージョンだけでなく、その
  エディションの **全リリース** を名指すようになりました。行がどちらであるかは新しい `targetable`
  列が示し、そのために `data_versions.schema_version` は `2` になります。`schema_version: 1` の表は
  従来どおり読め、意味も変わりません (v1 の行はすべてビルド可能なターゲットであり、この列は `true`
  が既定のため)。

  `@requires` の下限を順序付けることと、あるバージョン向けにビルドすることは別の問いです。前者に
  後者用の 3 行サンプルで答えていたことが、「パックが同梱していない実在のリリースを名指す下限」
  (Java の `version>=1.21.1`) と「他方のエディションのリリースを名指す下限」を区別できなくしていま
  した。前者は従来通っていたビルドを拒否し、しかも `1.21.1` は Java のリリースではないと述べていま
  した。実際にはリリースであり、パックがその `DataVersion` を持っていなかっただけです。どちらも
  直っています。下限は順序付けられ、ビルド対象のエディションのリリースを本当に名指していないラベル
  は従来どおり `E_REQUIRES_UNORDERABLE` です。

  Java の行はゲーム自身のバージョンメタデータ (リリースのみ、1.14 以降。`misode/mcmeta` の機械生成
  サマリ経由)、Bedrock の行は Mojang 自身の `bedrock-samples` のリリース一覧で、パレット整数は
  そのビルドの `(major << 24) | (minor << 16) | (patch << 8) | revision` として計算しています
  (既存の 3 行が完全に再現されることを確認済み)。`data_versions.json` は行の出所を新しい `source`
  フィールドに記録します。ローダはさらに、キーが一意な昇順でない表と、ラベルのテキスト順がキー順と
  一致しない表を拒否します。後者こそが「表の範囲の *外側* に下限を置く」判断が実際に依拠している
  性質で、従来ドックコメントが「各行のキーが何であれ」と述べていたものはこれではありませんでした。

  `--target`、`cairn info` の `buildable targets`、対応ターゲット一覧は変わりません。いずれも
  targetable な行を読みます。
...
- *(core)* `def` メンバの `E_UNRESOLVED_SLOT` が、**メンバごと・束縛テーマごとに高々 1 回** 報告
  されるようになりました。従来は placement の数だけ、さらにモジュールがテーマを自動選択できる
  場合はもう 1 回。def の本体はそれ自身のスコープとして 1 回、さらにインスタンス化する `place`
  ごとに 1 回解決されるため、テーマが 1 つのファイルで 2 箇所に置かれた def は同じ行・同じコード・
  ...
  は自分の対象テーマを名乗り、修正箇所も別です — こちらは両方とも残ります。記録するのは finding の
  同一性、すなわちメンバ・スロット名・テーマ名の三つ組で、しかも「本体を歩いたとき」ではなく
  「診断を積んだとき」に記録します。同じテーマを束縛した 2 つの解決が同じスロットについて別の判断に
  達しうる — 姉妹バリアントによるソフトニングは論理テーマ名の参照にだけ効き、バリアント名の参照には
  効かない — ので、何も言わなかった解決が次の解決を黙らせてはならないからです。食い違う 2 つの解決の
  どちらが残ったかは規定せず、`spec/lint` §11.1 にもそう書きました。
