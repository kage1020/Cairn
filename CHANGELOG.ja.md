...
- *(redstone)* 1 本のダストには 1 台のリピータ、そしてその分だけの遅延。Steiner ツリーはシンク
  ...
  一致しました。`logic sig.s0 = sig.a and sig.a` は 1 ブロックのダストに対して `wire_length: 2`
  と報告し、`delay_ticks` はその 1 本の上のリピータをポートごとに課金していました。どちらも
  ドライブしているネットを 1 回ずつ数えます。`BufferCoord` の doc は、その `{port, coord}` という
  形が元から持っていた読みを明文化しました — このベクタは attribution のリストであり、
  「セグメント × そのセグメントが通過するリピータ」1 つにつき 1 エントリです。したがって 1 つの
  ブロックが複数のセグメントを担うとき coord は繰り返され、ブロック数を数える利用者は coord で
  重複を除きます。

  これらの数値は公開されています。`cairn synth --stage route|delay|crossing` は `wire_length` /
  `delay_ticks` / `buffer_coords` を出力しており、それを読んでいる利用者には何の合図もなく値が
  変わります — Cargo は CalVer の `2026` をメジャーと読むため、月次のバンプで
  `>=2026.8.2, <2027.0.0` の範囲内に流れます。拒否されていたソースは exit 0 になります。それでも
  `修正` に置いているのは、旧来の数値が誰にも作れない配置を記述していたからです — 1 本のダストの
  上に 2 つのブロックが立ち、信号はそのそれぞれを 2 回以上通ったことにされていました。
