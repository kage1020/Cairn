...
- *(core)* `def` メンバの `E_UNRESOLVED_SLOT` が、「placement の数 + 1」回ではなく **メンバごと・
  テーマごとに 1 回** 報告されるようになりました。def の本体はそれ自身のスコープとして 1 回、さらに
  インスタンス化する `place` ごとに 1 回解決されるため、2 箇所に置かれた def は同じ行・同じコード・
  同じ note を 3 回報告し、しかもコピーを見分ける手がかりがありませんでした。その回数は、修正すべき
  ソースについての事実ではなく placement の並びについての事実です。

  一律の重複除去ではありません。解決どうしは互いのコピーではないからです。2 つの placement が別々の
  テーマを名指しすれば、同じ `mat_slot=` が別々のスロットマップに対して解決され、それぞれの finding
  は自分の対象テーマを名乗り、修正箇所も別です — こちらは両方とも残ります。記録するのはメンバとテーマ
  の組で、しかも「本体を歩いたとき」ではなく「診断を積んだとき」に記録します。スロットを束縛できた
  解決や、姉妹エディションのバリアントが宣言しているためスキップした解決が、あとから来るより厳しい
  解決を黙らせないためです。

  どこからも置かれない def は、モジュールが選んだテーマに対する finding を保ちます。def だけで `site`
  の無いファイルも検査に値しますし、本体に到達する前に打ち切られた placement — `theme=` が無い、
  origin が解決できない — では、def 自身の解決だけが本体の中身を報告できるからです。`spec/lint`
  §11.1 にコードと規則を追記しました。

- *(core,cli,lsp)* パースできないソースにも、他の finding と同じ診断が付くようになりました。新しい
  コード `E_PARSE` です。`--format json` は stdout に JSON を 1 つ返すと約束していながら、**最も
  多い失敗の仕方**であるパース失敗のときだけ stdout が空で、理由は散文として stderr に出ていました。
  stdout を読む利用者には何も届かず、終了コードから推測するしかありませんでした。

  抜け落ちていた理由は、パース失敗に「形」が無かったことです。コードは check と resolve のパスが
  生むもので、パース失敗はエラー型でしかありませんでした。表示したい側はそれぞれ自前の形を作って
  いました — CLI は素の `error:` 行を 5 箇所に複製し、language server はコードを持たない診断を直接
  組み立てていました。

  1 つのコードがパース失敗のあらゆる形をカバーし、どれだったかはメッセージが言います。形ごとに
  コードを分けると、将来増える形のたびに公開契約が 1 行増え、しかも分岐したい利用者はいません。
  span はパーサが報告した位置からその行末までです。位置だけではエディタ上で「下に何も無いキャレット」
  として描かれるためです。

  `cairn check --format json` は従来どおりの配列を、要素 1 つで出します。`cairn info --format json`
  の成果物は診断の一覧ではなくレポートなので、レポートが出せないときは代わりに
  `{"diagnostics": [...]}` を書きます — キーでレポートと区別できます。check レベルのエラーでも同じ
  で、こちらは 1 パス後に起きていた同じ沈黙でした。正常時のレポートは変わりません。

- *(tree-sitter)* すべての行が、文法上必ず必要なトークンから始まるようになりました。これにより、
  「レベルが変わると分かっている場所」だけでなく、**行が始まるあらゆる場所**でインデントが検査され
  ます。文法が受理していて `cairn-lang-core` が拒否する 2 つの形が、拒否されるようになりました —
  素材を束ねるだけで何も開かない `theme` の行の下にインデントされた行と、ディレクティブの下に
  インデントされた行です。どちらも**沈黙する方向**の誤りで、ファイルはパースされ、深く書かれた行は
  作者が書いた場所とは違うところに着地していました。

  このトークンは hidden かつ幅ゼロで、行から始まるすべての構文 — 各ディレクティブ、各トップレベル
  宣言、body の各項目 — の直前に立ちます。これを出さないことがそのまま拒否になります。文法上、
  構文を開始できるものが他に無いからです。indent / dedent トークンの**後ろ**に置いているのが要点で、
  前に置いても、直前の改行が既に見ている内容を二度見るだけになり、「1 段深い」が合法か違法かを
  判別できません。そこに body を開けるかどうかは文法の知識であって scanner の知識ではないためです。

  body を持たない宣言の後ろに、空行やコメント行を置けるようになりました。それらを跨ぐのは、scanner
  が「レベルを持つ次の行」を探して読み進む動きです。宣言の body はどれも optional なので、その位置
  でも indent が期待され、だから読み進めます — 新しいトークン自身は何も跨ぎません。これは以前は
  別個の拒否として記録されていたもので、パッチではなく改行処理の作り直しが必要だとされていました。
  ファイル末尾では、レイアウトの後ろに構文が無いため、いまも拒否のままです。
