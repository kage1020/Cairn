...
- *(core)* `@cairn` ヘッダの値が言語バージョンとして読まれるようになりました。コードは 2 つで、
  ...
  で書かれたファイルについて古いビルドが言える唯一の有用なことです。このビルドより後に追加された構文は
  `E_UNKNOWN_KEYWORD` や `E_UNKNOWN_ARGUMENT` として報告されるので、それらがその行についてではなく
  バージョン差についての指摘かもしれない、と知っているのはヘッダだけです。
  ...
  受け付ける形は 4 桁の年、`1 … 12` の月、省略可能なパッチです。月の先頭のゼロは受け付けられ、値を
  変えないので、`2026.06` (同梱の `.crn` も仕様自身もこう書きます。calver.org の `YYYY.0M` です) と
  `2026.6` は同じバージョンです。`@requires` が読むドット区切り 10 進より意図的に厳しくしてあります。
  あちらは Mojang の名前空間のラベルに順序を付けるだけですが、こちらは Cairn 自身のバージョンを指し、
  `2026.13` は存在しない月、`1.2` は年ではなく semver だからです。
