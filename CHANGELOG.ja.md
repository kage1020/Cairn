...
- *(redstone)* 各ネットが既に敷かれたダストを避けて配線されるようになり、2 つのネットが配線座標を
  ...
  領域の大きさによらず、です。配線を bridge 層へ持ち上げても解決しません。持ち上げた配線も結局は
  面を通って到達しなければならないからです。そこで配置パスは行を `x = 1 + 2i` に敷き、ルータは
  ...
  **破壊的**: セル座標が動くので、`cairn synth` のダンプにある `wire_length` / `delay_ticks` /
  `buffer_coords` はすべて変わります。セル数の 2 倍より狭い `circuit` 領域は配置パスが拒否し、
  行が必要とする列数をメッセージが名指します。`W_WIRE_CROSSING` / `E_CROSSING_CONGESTION` /
  `E_BUFFER_COORD_COLLISION` は削除されました — 前 2 つはコンパイラがその欠陥を作らなくなった
  ため、3 つ目はリピータが自分のネットの経路の上に立つようになり、それを奪い合う相手がいなくなった
  ためです。`examples/crossbar.crn` は `size=5x4` から `size=6x4` へ 1 列広がります。5 では
  最後のネットに行き場が無く、ショートさせるのではなくスコープが拒否されるからです。
