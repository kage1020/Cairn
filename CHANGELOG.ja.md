...
- *(cli)* `cairn check --format text` は診断を stderr に書きます。ビルド系コマンドの中でこれだけが
  ...
  いました。パースできないファイルに対する `--format json` も、stdout には何も出さず stderr に
  プレーンテキストを出すという、stdout をパースする側の期待と逆の挙動でした。`--format json` は
  stdout のままです — こちらは成果物であり、意図してリダイレクトされるものだからです。
...
- *(cli)* `cairn compile` は置き換える前のロックファイルを読み、何が変わったかを `spec` 10.6 の
  ...
  捏造は一切しないので、制約カタログの取り込みが入るまでこの行は黙ったままです。パースできない
