...
([principles P3](/spec/principles/)); inventing parallel terminology defeats the lint loop.
...
- **Block-array IR.** The universal pivot at the bottom of the three-layer IR: a voxel grid +
  ...
  [architecture §3.1](/spec/architecture/#31-the-block-array-ir-is-the-universal-pivot).
- **Intent IR.** The top layer: named members carrying `id` / `class` / `role` / `mat_slot` /
  ...
  [architecture §3.2](/spec/architecture/#32-the-intent-ir-is-rich-and-carries-invariants).
- **Semantic / Component-Theme IR.** The middle layer that resolves themes, components (`def`),
  ...
  [components-editing-sites](/spec/components-editing-sites/),
  [materials-themes](/spec/materials-themes/).
- **Logic IR / Netlist IR / Placement IR.** The three redstone sub-layers between Intent IR and
  ...
  [architecture §3.3](/spec/architecture/#33-redstone-sub-layers).
- **`semantic_level`.** An imported artifact's progress label: `raw` (one voxel per line) →
  ...
  the LLM's job. See
  [ecosystem-interop §12.3](/spec/ecosystem-interop/#123-three-tiers-of-transliteration).
...
- **`mat_slot`.** A material injection point on a member; bound by a `theme`. Structure carries
  ...
  [materials-themes §7.1](/spec/materials-themes/#71-slots-as-dependency-injection).
- **`intent_state`.** The author's blockstate intent. Edit diffs look **only** here. See
  [blockstate §6.2](/spec/blockstate/#62-intent_state-and-resolved_state).
...
- **Override-promotion.** Writing a blockstate that *could* be intent promotes it from
  ...
  [blockstate §6.1](/spec/blockstate/#61-derive-by-default-promote-on-override).
- **Anchor / bbox.** Every primitive carries an `anchor` (reference point), a declared bbox, an
  ...
  [entities §8.2](/spec/entities/#82-anchor-conventions).
...
- **Canonical token.** The value bound to a slot or theme selector: a *meaning* token, not a raw
  ...
  [materials-themes §7.2](/spec/materials-themes/#72-canonical-vocabulary).
...
- **Phase.** A fixed evaluation slot the compiler sorts each command into:
  ...
  ([compilation §4.1](/spec/compilation/#41-phase-evaluation)).
- **Last-wins (local).** Within the same phase, a later command overrides an earlier one. The old
  whole-program last-wins ("paint model") is dropped. See [principles P2](/spec/principles/).
- **Target axes.** `(edition, version)`: the only layer that knows them is the backend. The DSL
  ...
  [compilation §4.2](/spec/compilation/#42-target-axes), [versioning-editions](/spec/versioning-editions/).
- **DataVersion.** Mojang's monotonically increasing integer key for a Java version. Cairn uses it
  ...
  [versioning-editions §10.1](/spec/versioning-editions/#101-the-target-is-a-compile-time-parameter).
...
- **Lock (`*.cairn.lock`).** Compiler-generated reproducibility record. Carries `source_hash`,
  ...
  [versioning-editions §10.6](/spec/versioning-editions/#106-provenance-and-lock).
...
- **Logical cell / edition cell / physical tile.** The three-tier cell library. The Logic IR
  ...
  Java/Bedrock difference to the library. See [redstone §14.6](/spec/redstone/#146-edition-differences).
...
- **Truth / latency / temporal assertion.** The three verification kinds. See
  [redstone §14.7](/spec/redstone/#147-verification). Temporal is bounded `eventually within N` only, not
  full LTL.
...
- **Self-correction loop.** The compile → diagnostics → patch → recompile cycle that lifts
  precision out of one-shot generation ([principles P5](/spec/principles/)). Diagnostic messages MUST
...
- **Fail-loud.** Silent substitution and implicit dropping of unknown IDs / out-of-domain states
  ...
  See [versioning-editions §10.4](/spec/versioning-editions/#104-fail-loud-and-minimum-version-inference).
- **`semantic_sensitivity`.** A constraint-catalog field distinguishing "the ID stays valid but
  ...
  [versioning-editions §10.5](/spec/versioning-editions/#105-which-version-is-it-for-has-three-answers).
- **Block IoU.** Voxel intersection-over-union used for the import self-correction loop. Convergence
  threshold ≥ 0.985. See
  [ecosystem-interop §12.2](/spec/ecosystem-interop/#122-reverse-direction-the-compiler-transliterates-an-llm-lifts),
  [evaluation §13.2](/spec/evaluation/#132-reverse-conversion-auxiliary-metrics).
- **Zero-shot Compile Rate / Fix Convergence Rate / Token Efficiency / Edit Stability.** The four
  primary spec-iteration metrics. See [evaluation §13.1](/spec/evaluation/#131-primary-metrics).

## Editions and versions

- **Edition.** `java` or `bedrock`. Compile-time only; never present in the DSL semantic layer.
  See [versioning-editions §10.7](/spec/versioning-editions/#107-java--bedrock-portability).
- **Cairn version vs MC target.** Two **separate axes** distinguished by field/flag/keyword, never
  ...
  [Specification overview](/spec/), [versioning-editions](/spec/versioning-editions/).
- **Recompile, don't transcode.** The language spec does **not** guarantee NBT portability across
  ...
  [versioning-editions §10.2](/spec/versioning-editions/#102-language-contract-recompile-dont-transcode).
...
- **Raw / L1 / L2.** The three tiers of faithful transliteration on import. The compiler reaches
  ...
  `theme`). See [ecosystem-interop §12.3](/spec/ecosystem-interop/#123-three-tiers-of-transliteration).
