...
**The type of the reverse-direction IR** — one IR that can degenerate, or separate Intent IR and Raw
Geometry IR types. The tentative policy is to share the block-array layer and split types at the
...
"edition" mechanism with year-based opt-in, or simply announcing changes per release in the
CHANGELOG. Note that "edition" already means Java or Bedrock here, so a different term would be
needed. For now the latter — CalVer plus `@cairn` provenance — suffices.
