...
- The reverse direction (NBT/schematic → Cairn) is best-effort ([Ecosystem Interop](ecosystem-interop)).
- The portable artifact is always the **Cairn source**; emitted NBT/schematics are per-target build
  outputs (the equivalent of a binary) ([Versioning and Editions](versioning-editions)).

## 1.3 Scope and non-goals
- Redstone **can be described at the logic level** (declare signals, gates, and connections; the
  compiler synthesizes and place-and-routes; see [Redstone](redstone)). v1 is limited to
