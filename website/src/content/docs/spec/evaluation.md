...
|---|---|
| **Zero-shot compile rate** | The fraction that compiles error-free on the first try — how intuitive the spec is. |
| **Fix convergence rate** | The fraction that converges to error-free within 3 turns — how expressive the error messages are. |
...
- `residual_ratio` — raw volume left after the lift. `compression_ratio` — voxel count ÷ token
  count.
...
metrics, especially fix convergence rate and edit stability. Running the experiment — give the model
only the spec, have it generate, and observe where the errors are — settles most syntax and
vocabulary questions on real data.
