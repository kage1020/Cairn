...
`E_DUPLICATE_ITEM` になります — 解決されるのは最初の宣言なので、報告しなければ 2 つ目は無言で
ビルドから落ちます。
...
- 各ディレクティブは 1 モジュールにつき **1 回まで** です → `E_DUPLICATE_HEADER`。繰り返しても
  加算されません: `@cairn` と `@intended_targets` は読み手が 1 箇所ずつ、`@requires` の下限は最も
  厳しいものに畳み込まれるため、2 行目がより緩い内容を述べていてもビルドには一切残りません。
