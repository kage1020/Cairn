...
**ポートは壁が開けられた場所です。** ポートを持つ 2 つのロールはどちらも石積みを貫く開口なので、壁
の段を 1 つも塗らない placement はどちらのポートも支えません。`door` は切り抜く層が 1 つ要り、
`window` は矩形全体が 1 つの層の中に収まっていなければなりません。
...
`W_DEFERRED_MEMBER` とともに落ち、note には door / window / 石積み / 予約ロールの契約が順に並びます。
足りないのが石積みである場合、建てられなかったメンバ自身がその行でそう述べます。
