...
`id=` は §5.5 のジオメトリメンバと違い、自動採番ではなく必須です。メンバの自動アドレスは本体内での
役割と位置から導かれますが、`place` の名前はコンパイラが書き出すファイル名であり、`east_of=` や
`connect` が参照するスコープキーの後半でもあります。自動生成した名前は、作者が書いておらず参照も
できない名前になってしまいます。
