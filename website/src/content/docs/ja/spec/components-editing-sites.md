...
`walls height=H` はワールドの段 `1 … H` を埋め、段 `0` は床スラブのものです。壁の外に段がはみ出す窓
は walkway を支えられず、その行は `W_DEFERRED_MEMBER` とともに落ちます。note には door / window / 予
約ロールの契約が順に並びます。

現時点でポートと開口が食い違う場面が 1 つあります。ポートは `walls` メンバが *宣言する* 段を読み、開
口パスは実際に *塗る* 段を読みます。`mat_slot=` が解決しない `walls` では、切り抜きは保留されますが
ポートは張り付いたままです。
...
囲われている、site がルータの探索面積上限を超えている — に限り、直線の L 字に戻して衝突するセルを飛
ばします。このとき `W_WALKWAY_BLOCKED` が 1 件出て、具体的な原因とその修復 (埋まったドアや窓を動かす、
