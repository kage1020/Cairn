...
1 つだけ名指しします: ポートを持てないロール、書かれた `side=` や `at=`、欠けている window の引数、
矩形が壁からはみ出した量、あるいは石積みが覆っている段と覆っていない段です。note はそのメンバの行を
指し、問題がロールではなくメンバの引数や石積みにある場合は、開口も切られていないため、その行にメンバ
自身の指摘も並びます。`place` によって座標の範囲外へ押し出されたポートは指すべきメンバの行を持たず、
行の上だけでそう述べます。
