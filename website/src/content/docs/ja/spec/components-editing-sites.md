...
### 9.3.3 スコープ跨ぎ参照
`place` 行は `id=` / `use=` / `theme=` をすべて宣言します。いずれかを欠いた行は配置になれません —
`.nbt` の名前がない (§9.3.4)、インスタンス化する `def` がない、`mat_slot=` を解決するテーマがない —
ので `E_INCOMPLETE_PLACE` となり、その行はビルドから落ちます。メッセージは不足しているキーを
すべて列挙します。キーが存在してラベルでないだけの場合 (`use=3`) は `E_TYPE_MISMATCH_LABEL` です。
行の上にはあるが使えない、という別の問題だからです。

`id=` は §9.2 のジオメトリメンバと違い、自動付与ではなく必須です。自動アドレスは
parent / role / side / level / offset から導かれ、その本体の外では何も指しません。一方 `place` の
`id=` は `east_of=` や `connect` が参照する名前であり、`.nbt` のファイル名 (§9.3.4) でもあります。
自動生成した名前は、作者が書いておらず参照もできない名前になってしまいます。
...
います。L 字が既存の構造床を横切る場合、コンパイラは代わりに地面平面で迂回路を探索します:
障害物を回る最短経路、同じ長さなら曲がり回数が最少の経路を選び、タイブレークは決定的なので
同じソースは常に同じ strip を敷設します (lockfile の再現性要件)。ポートが他の placement の床に
埋まっている、到達先が完全に囲まれている、site が探索面積上限を超えているなど、遮られない経路が
まったく存在しない場合に限り、行は直進 L にフォールバックして衝突セルをスキップし、
`W_WALKWAY_BLOCKED` を 1 件だけ警告として発します。note は迂回探索が突き当たった具体的な原因と
それに効く対処法 (埋まった door/window の移動、gap の拡大、構造物同士を近づける) を書き分けます。
警告には `--format json` 出力で
機械可読ペイロード (`data: { kind: "walkway_blocked", skipped: N }`) が付与され、LSP の
クイックフィックスや CI アノテーターが人間向けメッセージを再パースせずにスキップ件数を
読み取れます — 詳細は `spec/lint.md` §11.2 を参照。
...
- `E_UNRESOLVED_PLACE_REF` — ドット左側の place ID が同一 site の先行 place を指していない。
  §9.3.3 と共有のコードです。
- `W_WALKWAY_BLOCKED` — 2 つのポート間に遮られない経路が存在しない。行は直進 L に
  フォールバックし、衝突セルはスキップされ、残りの経路はそのまま敷設されます。note は迂回探索が
  突き当たった原因 — ポートの埋没、到達先の閉塞、探索面積上限 — と、それに実際に効く対処法を
  書き分けます。JSON ペイロードはスキップ件数を `data.skipped` として公開するため、ツール側は
  メッセージ本文を再パースする必要がありません。
