...
1. **配置**: トポロジカル順、左から右へ。セル同士の間に 1 列、入力パッドの列と行の間にも 1 列の
   空きを置きます。セル本体はブロックなので、ネットはその隣接座標を通って届きます。2 入力ゲートには
   3 つの異なるネット (2 本のドライバと自分の出力) が接するので、空いた隣接座標が 3 つ必要です。
   詰めて並べると行の途中のセルには 2 つしか残らず、領域をいくら広げても増えません。行を空けるのは
   そのためで、交差の無い配線を *可能にする* ものであって保証するものではありません。通りすがりの
   ネットが最後の 1 面を取ることはあり、そのスコープはショートさせずに拒否されます。
2. **Steiner 配線**: Manhattan で、既に立っているもの、そして既に敷かれたネットのダストを避けて。
   ...
   1 つの座標に 2 つのネットが乗れば、2 つの信号を運ぶ 1 本のダストになります。そこでネットは
   1 本ずつ敷かれ、後のものが先のものを避けます。これが交差の脱出であり、段階 4 ではなくここで
   行うのは登った分を測るためです。`wire_length` もディレイパスのティック数も、この経路木から
   読み取られます。順序は fanout の降順、次にネット自身のキー — 全順序なので、1 つのレイアウトに
   対する答えは何度尋ねても 1 つです。
...
4. **交差の合法化**: 段階 3 が数えたバッファのリピータに座標を割り当てます。この時点で配線を
   ...
   ネットだけのものなので、持ち上げるべき交差も、奪い合う座標もありません。
5. **エディションの合法化**: [§14.6](#146-エディション差) を参照。

配線は `circuit` 領域に閉じ込められます。収まらなければ fail-loud です。ドライバからの経路がすべて
塞がれているシンク — コンポーネント、先に敷かれたネットのダスト、あるいは予約の端で — も、理由は
違えど同じ拒否で、結べなかった 2 つの座標を告げます。
