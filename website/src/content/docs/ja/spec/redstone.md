...
(`E_LOGIC_INVALID_SIGNAL`)。`opened_by=a` は `a` という信号への配線ではなく、何にも繋がって
いないドアです。

バインディングは `[selector]` の **後ろ** に書き、中には書きません。
`door[id=front] opened_by=sig.power` は結線し、`door[id=front,opened_by=sig.power]` は
結線しません。角括弧はその行が対象とするメンバを選ぶためのものなので、その中のバインディングは
書かれた場所で誰にも読まれないという点で誤ったコンポーネント上のものと同じ理由により
`E_LOGIC_MISPLACED_BINDING` になります。
