...
せん。edition 中立のゲートが union で吸収してしまうものを厳密な edition 別 dry-run は見ており、そこ
で上がった拒否も、要求された edition をすべて歩き終えたあとに同じドキュメントを書きます。レポートが
出せる実行における警告は、どちらのフォーマットでも stderr にテキストで報告されます。レポートが出せ
ない実行で edition 別パスが上げた警告も同じです。
...
ブロックステートをパレットが抱えている場合、その edition は portability の行を失いますが、これは
ソース中のスパンにも、作者に可能な修正にも紐づきません。どちらの形式でも stderr にテキストで読め
ます。それでもドキュメントは書かれます — 約束は「入力ごとに 1 ドキュメント」であって「拒否ごとに
1 要素」ではないからです。ほかに何も拒否がない実行は `{"diagnostics": []}` を書き、残りは終了
コードで伝えます。
