...
`E_UNRESOLVED_SLOT` は **メンバごと・テーマごとに 1 回** 報告されます。`def` の本体は、それ自身の
スコープとして 1 回、さらにインスタンス化する `place` ごとに 1 回解決され、そのどれもがテーマを
束縛します。同じテーマを束縛した解決は同じ行について同じ結論に達するので、まとめて 1 つの
finding です。2 つの placement が別々のテーマを名指しした場合は 2 つの finding になります。
それぞれが自分の対象テーマを名乗り、修正箇所も別だからです。どこからも置かれない `def` も、
モジュールが選んだテーマに対して解決されます。def だけで `site` の無いファイルも検査対象です。
