...
モジュールが選んだテーマに対して解決されます。したがって def だけで `site` の無いファイルも、
モジュールが選べる論理テーマが 1 つであるかぎり検査されます。2 つ以上宣言されている場合は
def 自身のスコープにテーマが束縛されないため、その `mat_slot=` は `place` がテーマを選ぶまで
判定されません。
