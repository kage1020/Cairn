...
上のブロックは stdout です。各数字の中身は stderr に `note:` 行として出ます — floor の下にある
ターゲット、拒否するバージョンとその理由、`unsupported` に数えられたパレットエントリ。行を読む
パイプラインは、`cairn info` が最後まで走るたびに同じ 4 行を見ます。走り切らない場合は別で、findings が
行を計算する前にコマンドを拒否し、stdout は 1 行足りないのではなく空になります。

`edition portability` の行はパレットエントリを数えます。エントリが `unsupported` になる理由は 4 つ
あります。

| 理由 | 直し方 |
| --- | --- |
| そのエディションにブロック自体が無い | マテリアル、またはパックの対応付けを変える |
| ブロックはあり、意図が持つ状態にこのコンパイラが対応付けを持たない (10.7) | いまは無い — 対応付けを足すのは Cairn 側 |
| Java のドメイン外の状態値が state translator まで届いた | 無い — パックが弾くべきだが、値のドメインを表現できるパックスキーマは今日存在しない |
| translator が読まない状態キーが届いた | ソースの blockstate からそのキーを消す |

1 つ目は ID の問いで、残りは状態の問いです。`degraded` を生むのは 2 つ目だけです — 存在しないブロック
には失われる詳細がなく、translator が丸ごと拒否した状態も部分的な損失ではありません。3 つ目と 4 つ目は
ポータビリティの事実ではありません。エディションに何かが欠けているからではなく、上流が blockstate を
通してしまったからそのエントリは unsupported なのです。

直し方が 4 通りある。だから数字だけでは動きようがありません。数えられたエントリはその下に stderr で
1 件ずつ名指しされ、理由と、ID のケースでは `E_UNKNOWN_ID` と同じ読み方 (単一の名前空間の中で path を
比較する) の `did you mean` が付きます。候補は違います。固定したビルドは 1 バージョンの表に、この行は
エディションが宣言する全バージョンに問うので、両者が別のブロックを挙げることがあり、それぞれ自分の問い
については正しい。`--format json` では `edition_portability[].unsupported_entries` に、カウント 1 に
つき 1 要素、パレット順で、理由のタグとその理由自身のフィールドを並べて載ります。
...
`cairn info` は exit 0 を返します。拒否するのはビルドのほうです。どのブロックが無いのかは、どちらでも
言われます。`cairn info` は拒否した各バージョンの findings をそのバージョンの下に出すので、`E_UNKNOWN_ID`
がそれを出したターゲットなしに単独で立つことはありません。`recommended test targets` はさらに別の問い —
どのバージョンで試すべきか であって、どれが動くか ではありません。
