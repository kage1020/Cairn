...
- **`place ... theme=NAME` は論理テーマを指し**、同じ規則に従います。1 つの site が同じ def を、ビルドが
  ...
  ます — 固定されたエディションのバリアントが束縛され、代わりに何が束縛されたかを
  `W_THEME_VARIANT_REBOUND` が伝えます — が、意味層が持つべきは中立な綴りです。`--edition` の固定がない
  場合、書かれた名前がそのまま束縛されます。別のバリアントを主張するエディションが存在しないからです。
