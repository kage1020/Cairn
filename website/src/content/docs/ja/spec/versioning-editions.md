...
パイプラインは、ソースが何であれ同じ 4 行を見ます。

`edition portability` の行はパレットエントリを数えます。エントリが `unsupported` になる理由は 3 つあり、
そのエディションにそのブロック自体が存在しない、ブロックはあっても意図が持つ状態を表現できない
(10.7)、レジストリパックが弾くべき状態が state translator まで届いた、のいずれかです。1 つ目は ID の
問いで、2 つ目は状態の問いです。`degraded` を生むのは 2 つ目だけです — 存在しないブロックには失われる
詳細がありません。3 つ目はそもそもソースについての事実ではありません。直すべきはパックであり、作者に
それはできません。

3 つは 3 つとも直し方が違います。だから数字だけでは動きようがありません。数えられたエントリはその下に
stderr で 1 件ずつ名指しされ、理由と、ID のケースでは `E_UNKNOWN_ID` が出すのと同じ `did you mean`
候補 (そのエディションが宣言する全 ID から引いたもの) が付きます。`--format json` では
`edition_portability[].unsupported_entries` に、カウント 1 につき 1 要素、パレット順で載ります。
