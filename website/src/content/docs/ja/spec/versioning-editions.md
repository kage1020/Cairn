...
`DataVersion` と比べられます。ビルドの可否を決めるためにバージョン文字列同士を比較することはあり
ません。

表が名指すのはパックがビルドされた対象バージョンだけで、下限は任意のバージョンを名指せるので、位置
づけは単純な参照では済みません。答えは 4 つあり、厳密なのは最初の 1 つだけです。
...
| 全行より下、または全行より上に位置する | 全ターゲットが満たす / どのターゲットも満たさない | 各行のキーが何であれ成り立つ。判定はラベルの比較なので、同じ採番体系のラベル同士でのみ信頼する。ドット区切り 10 進を日付ベースの行と比べる形では位置づけない。 |
...
の最新リリースは Bedrock のどのリリースも名指さず、`1.21.0` と `1.21.40` の間に位置します。ラベルの
...
`DataVersion` を持つかどうかは表の答えであり、エディションとターゲットの両方をピンする唯一のコマンド
である `cairn compile --target` で問われます。`cairn check` はどちらもピンしないので問いません。
