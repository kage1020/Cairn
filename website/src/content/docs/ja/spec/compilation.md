...
ありません。成果物についても同様です ([§4.8](#48-同一フェーズ内の衝突とパレット))。ボクセルを共有
しない 2 つのメンバを入れ替えても、ボクセルだけでなくパレットの番号付けを含む `BlockArray` 全体が
等しいままです。
...
- 値が変わらないセル。同じ材質の `walls` 2 枚が共有する段で重なる場合です。
- メンバが自分自身の上に書く場合。`repeat=` / `step=` のスタンプが重なる `window` などです。

**パレット** は、フェーズが評価する本体 (`struct`、`def`、それらを実体化する各 `place`) にとって、
書き込みの履歴ではなく *正準な並べ方を持つ集合* です。その本体が含むブロックを列挙し、並び順は 1 つに
固定されます。スロット `0` は空気、それ以外は `(id, properties)` の昇順です。

まず id が決めます。id が同じ 2 つでは、それぞれのプロパティを名前順に並べたうえで 2 つのリストを
先頭から対ごとに比較します。したがって名前が一致した時点で**値**が決め (`facing=north` が
`facing=south` より前)、短い袋は、それを先頭に含むより長い袋より前に来ます
(`oak_stairs[facing=north]` が `oak_stairs[facing=north,half=top]` より前)。名前でソートすることが、
状態リテラルがプロパティをどう並べて書いたかから順序を切り離します。
`@oak_stairs[half=top,facing=north]` と `@oak_stairs[facing=north,half=top]` は同じ 1 つのブロックで
あり、テーマがどちらを先に書いたかがスロットを動かしてはならないからです。

「その本体が含むブロック」の例外は 2 つです。スロット `0` の空気は何があっても残るので、隙間なく
埋まった体積でもこのエントリを持ちます。もう 1 つは、そもそも一度も塗られなかったスロットで、
これは意図的に残します。削除してしまうと、ジェネレータが出力しない形状のために材質を intern した
という事実の、リリースされたビルドが運ぶ唯一の証拠を消すことになるからです。

ここから 2 つの規則が導かれます。どちらも、この番号付けが `.nbt`、`cairn info` のエントリ単位の行、
`resolved_ir_hash` に届くために存在します。

- 最後の 1 ボクセルを後続フェーズに覆われた材質は**削除**され、残りのスロットは詰め直されます。
  そうしなければ「どちらのメンバが負けたか」だけが異なる 2 つのソースが、同じビルドに対して別々の
  成果物を生むことになります。
- ボクセルを共有しない 2 つのメンバは互いを順序づけません。完成したグリッドはどちらの行が先かに依存
  しないのだから、成果物も依存してはならない — それが §4.1 冒頭の約束であり、ボクセルだけでなく
  バイト列についての主張です。ソートがそれを真にします。パレットは完成したグリッドだけから導かれる
  ので、グリッドを変えないソースの並べ替えは `BlockArray` 全体を変えません。

ソートの対象は解決済みの状態です。ソースのスパン、メンバ id、フェーズ、塗った順のいずれも参照しない
という意味で、エディション・バージョンをまたいで安定です。2 つのターゲットが同じパレットを持つという
主張ではありません。そこに載る id 自体が一致するとは限らないからです。
