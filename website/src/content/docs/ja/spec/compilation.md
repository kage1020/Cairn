...
- 所属は上の一覧のとおりです。`floor` と `walls` は massing、`roof` は envelope で、軒が外装である
  `stair` (§4.3) も同じです。`door` と `window` は openings、`pressure_plate` はセンサなので
...
も、結果は順序に依存しません)。`sym=true` の窓の左右のように、1 つのメンバが自分自身
の上に書く場合も同様です。

完成した構造のパレットは、その構造が含むブロックを、各フェーズが最初に塗った順で並べ
たものであり、スロット `0` は空気です。途中で intern された履歴ではありません。最後の
