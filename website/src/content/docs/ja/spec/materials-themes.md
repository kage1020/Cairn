...
両方が束ねるキーは先の行のものを読むものが 1 つも残りません。これが `E_DUPLICATE_SELECTOR` です ([Lint §11.1](/ja/spec/lint/#111-診断コード))。
...
とに ID、ステート名、ステート値、シリアライズを解決するのはバックエンドです ([バージョンとエディション](/ja/spec/versioning-editions/))。
...
[バージョンとエディション](/ja/spec/versioning-editions/) にあります。
