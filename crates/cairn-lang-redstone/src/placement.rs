...
fn compile_scope(
    ...
    if source.cells.is_empty() {
        debug_assert!(
            source.is_empty(),
            "EditionNetlistIr with cells==0 but non-empty inputs/outputs slipped through elision",
        );
        return Ok(ir);
    }
...
fn missing_region_diagnostic(source: &EditionNetlistIr) -> Diagnostic {
    ...
    Diagnostic::new(
        ...
        span,
        "this scope has redstone cells to place but declares no `circuit region=<label> void=<N>` reservation"
            .to_owned(),
    )
    .with_footer(
        "add a `circuit region=<label> void=<N>` line whose `region=` names a member kind that lives in the scope's footprint, and give the enclosing scope a `size=WxH` header",
    )
}
...
fn congestion_diagnostic(
    ...
    let ratio_x10 = (required_area * 10) / reserved_area.max(1);
...
fn build_region_index(
    ...
    for region in intent::circuit_regions(intent) {
        ...
        // The routing pass owns the "split into multiple circuit
        // blocks" hint per `spec/redstone` §14.5, so a warning for
        // duplicates joins that pass, not this one.
