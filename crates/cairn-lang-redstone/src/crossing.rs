...
//! [`crate::routing_geometry::net_trees`] — the same call the routing
...
//!    picked off
//!    [`crate::routing_geometry::NetTree::route_to`] — the routed path
...
//!    [`crate::delay::buffer_count_for_segment`], the function the
...
fn legalize_scope(entry: &ScopedPlacementIrEntry) -> ScopeLegalization {
    ...
    // A coord with two or more owners would short in the voxel model.
    // The first two in net order name the pair the diagnostic blames;
    // a third net through the same coord does not change it. Sorted by
    // (x, z, y) so the anchor does not drift across runs.
...
mod tests {
    ...
    /// Every buffer stands on wire its own net owns: on the plane, a
    /// coord of the route from the driver's source to that cell; on a
    /// bridge, the layer directly above one. This is the invariant the
    /// pass exists to hold, asserted against the same
    /// [`crate::routing_geometry`] the production path reads so a
    /// return to a hand-rolled straight line fails here.
    #[test]
    fn every_buffer_stands_on_its_own_nets_wire() {
        let legalized = compile_crossing(&detour_scope(2));
        ...
        for (index, cell) in ir.cells.iter().enumerate() {
            for buffer in cell.buffer_coords() {
                ...
                let route: HashSet<CellCoord> = trees[&driver.net]
                    .route_to(cell_coords[index])
                    .expect("the cell is a terminal of the net driving it")
                    .into_iter()
                    .collect();
                let footprint = CellCoord::new(buffer.coord.x, 0, buffer.coord.z);
                assert!(
                    route.contains(&footprint),
                    "buffer {:?} on cell #{index} is not over its own net's route",
                    buffer.coord,
                );
                ...
                for (other, tree) in &trees {
                    if *other == driver.net || buffer.coord.layer != RouteLayer::Plane {
                        continue;
                    }
                    assert!(
                        !tree.wire_path().contains(&buffer.coord),
