...
**A breaking change puts `!` after the scope** — `feat(core)!: replace lexer`, `fix(redstone)!: route each net around the dust already laid`. The `!` and the changelog go together: a PR that adds an entry under `## [Unreleased]` → `### Breaking changes` in [CHANGELOG.md](CHANGELOG.md) titles itself with `!`, and a PR titled with `!` adds that entry. The hand-written entry is what [How a break is communicated](https://cairn.kage1020.com/spec/compatibility/#c3-how-a-break-is-communicated) requires and what a consumer reads; the `!` is the same fact in the commit `release-plz` parses. A reviewer who sees one without the other asks for the missing half before merging.
